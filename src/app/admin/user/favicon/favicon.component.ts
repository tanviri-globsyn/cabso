...
export class FaviconComponent implements OnInit {
  ...
  createForm(){
    this.Form = new FormGroup({
      favIcon: new FormControl("", [FileValidator.validate])
  });
  ...
  addFavi(Form){
   ...
   let files = this.elem.nativeElement.querySelector('#favIcon').files;
   if (!this.validateFile(files[0].name)) {
    this.toastr.error('Support .png files only');  
   
  } else { 
          let formData = new FormData();
          let file = files[0];
          formData.append('favIcon', file, file.name);
         this.service.updateFavicon(formData,params['id']).subscribe(data=>{ 
          if(data.success) {
            ...
            this.router.navigate(['admin/user/favicon/'+params['id']]);
            window.scrollTo(0, 0);
          }
